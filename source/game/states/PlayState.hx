...
import game.editors.CharacterEditorState;
...
import visuals.objects.*;
import visuals.stages.*;
import visuals.stages.objects.*;

#if LUA_ALLOWED
import utils.scripting.songs.*;
#else
...
class PlayState extends MusicBeatState
{
	...
	public var healthBarPercent:Float = 1;
	...
	public var scoreTxtData:Array<String> = [];
	...
	private var luaDebugGroup:FlxTypedGroup<utils.scripting.songs.DebugLuaText>;
	#end
	...
	public static var onOptionsState:Bool = false;

	public var camGameModulo:Int = 4;
	public var camHUDModulo:Int = 4;
	public var iconsModulo:Int = 1;
	...
	override public function create()
	{
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		luaDebugGroup = new FlxTypedGroup<utils.scripting.songs.DebugLuaText>();
		...
		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.12), 'healthBar', function() return healthBarPercent, 0, 2);
		...
		scoreTxt = new FlxText(0, healthBar.y + 37, FlxG.width, "", 16);
		...
		scoreTxt.applyMarkup(tempScore, [new FlxTextFormatMarkerPair(new FlxTextFormat(CoolUtil.colorFromString('909090')), '*')]);

		botplayTxt = new FlxText(0, ClientPrefs.data.downScroll ? 605 : 85, FlxG.width, 'BOTPLAY', 32);
	...
	public function addTextToDebug(text:String, color:FlxColor) {
		var newText:utils.scripting.songs.DebugLuaText = luaDebugGroup.recycle(utils.scripting.songs.DebugLuaText);
		newText.text = text;
		newText.color = color;
		...
		luaDebugGroup.forEachAlive(function(spr:utils.scripting.songs.DebugLuaText) {
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		if(totalPlayed == 0)
		{
			...
			accuracy = percent + '% - ' + ratingFC[0];
		}

		if (instakillOnMiss)
		{
			tempScore = 'Score: ' + songScore + '      ' + 'Rating: ' + accuracy;
		} else {
			tempScore = 'Score: ' + songScore + '      ' + 'Misses: ' + songMisses + '      ' + 'Rating: ' + accuracy;
		}

		scoreTxtData = [tempScore, ratingFC[1]];
	...
	public dynamic function updateScoreText()
	{
		if (scoreTxt != null)
		{
			scoreTxt.applyMarkup(scoreTxtData[0], [new FlxTextFormatMarkerPair(new FlxTextFormat(CoolUtil.colorFromString(scoreTxtData[1])), '*')]);
		}
	}
	...
	public dynamic function fullComboFunction()
	{
		...
		if (totalPlayed == 0)
		{
			...
			if (songMisses == 0)
			{
				...
				if (songMisses < 10) 
				{
					ratingFC = ['*SDCB*', 'FFFF00'];
				} else {
					ratingFC = ['*Clear*', 'FF5F5F'];
				}
			}
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		healthBarPercent = CoolUtil.fpsLerp(healthBarPercent, health, 0.2);
	...
	override function destroy()
	{
		...
		visuals.objects.NoteTypesConfig.clearNoteTypesData();
	...
	override function beatHit()
	{
		...
		if (camZooming && curBeat % camGameModulo == 0 && ClientPrefs.data.camZooms) FlxG.camera.zoom += 0.015 * camZoomingMult;

		if (camZooming && curBeat % camHUDModulo == 0 && ClientPrefs.data.camZooms) camHUD.zoom += 0.03 * camZoomingMult;

		if (curBeat % iconsModulo == 0)
		{
			iconP1.scale.x = 1.2;
			iconP1.scale.y = 1.2;
			iconP1.updateHitbox();
	...
	public function playerDance():Void
	{
		...
		if(FlxG.sound.music != null && boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 #if FLX_PITCH / FlxG.sound.music.pitch #end) * boyfriend.singDuration && anim.startsWith('sing') && !anim.endsWith('miss'))
			boyfriend.dance();
	}
