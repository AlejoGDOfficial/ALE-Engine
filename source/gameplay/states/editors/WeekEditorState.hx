...
class WeekEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		bgSprite.antialiasing = ClientPrefs.getJsonPref('antiAliasing');

		weekThing = new MenuItem(0, bgSprite.y + 396, weekFileName);
		weekThing.y += weekThing.height + 20;
		weekThing.antialiasing = ClientPrefs.getJsonPref('antiAliasing');
		add(weekThing);
		...
		lock.antialiasing = ClientPrefs.getJsonPref('antiAliasing');
		add(lock);
		...
		tracksSprite.antialiasing = ClientPrefs.getJsonPref('antiAliasing');
		add(tracksSprite);
