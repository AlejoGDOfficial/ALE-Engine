...
class ResetScoreSubState extends MusicBeatSubstate
{
	...
	public function new(song:String, difficulty:Int, character:String, week:Int = -1)
	{
		...
		var text:Alphabet = new Alphabet(0, 180, LanguageManager.getPhrase('freeplayStateResetScore'), true);
		text.screenCenter(X);
		...
		yesText = new Alphabet(0, text.y + 150, LanguageManager.getPhrase('freeplayStateYes'), true);
		yesText.screenCenter(X);
		yesText.x -= 200;
		add(yesText);
		noText = new Alphabet(0, text.y + 150, LanguageManager.getPhrase('freeplayStateNo'), true);
