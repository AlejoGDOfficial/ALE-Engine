	...
	override function update(elapsed:Float)
	{
		if(controls.BACK && hoveringOnMods)
		{
			...
			if(waitingToRestart)
			{
				...
				{
					FlxTween.cancelTweensOf(mustSelectOneMod);
					mustSelectOneMod.alpha = 1;
					FlxTween.tween(mustSelectOneMod, {alpha: 0}, 1);
				} else {
					var closeNumTween = FlxTween.num(1, 0, 1, {
					...
					closeNumTween.onUpdate = function(twn:FlxTween)
					{
						WindowsCPP.setWindowAlpha(closeNumTween.value);
					}
				}
			}
