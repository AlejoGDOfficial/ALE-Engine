...
class ALEFunctions
{
	...
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "addBackdrop", function(tag:String, image:String)
		{
			...
			ScriptState.instance.add(backDrop);
			ScriptState.instance.variables.set(tag, backDrop);
		});
		Lua_helper.add_callback(lua, "removeBackdrop", function(tag:String)
		{
			ScriptState.instance.variables[tag].destroy();
			ScriptState.instance.variables.remove(tag);
		
			ScriptState.instance.remove(ScriptState.instance.variables.get(tag));
		});
		Lua_helper.add_callback(lua, "addCamera", function(tag:String, defaultTarget:Bool, x:Int, y:Int, height:Int, zoom:Float)
		{
			...
			ScriptState.instance.variables.set(tag, camera);
		});
		Lua_helper.add_callback(lua, "removeCamera", function(tag:String)
		{
			var obj:Dynamic = ScriptState.instance.variables.get(tag);

			if (Std.isOfType(obj, FlxCamera))
			{
				FlxG.cameras.remove(cast obj);
				ScriptState.instance.variables.remove(obj);
			}
		});
