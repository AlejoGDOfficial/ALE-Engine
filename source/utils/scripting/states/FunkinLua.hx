...
import utils.scripting.states.LuaUtils.LuaTweenOptions;
#if SScript
...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		Lua_helper.add_callback(lua, "setVar", function(varName:String, value:Dynamic) {
			ScriptState.instance.variables.set(varName, value);
			return value;
		});
		Lua_helper.add_callback(lua, "getVar", function(varName:String) {
			return ScriptState.instance.variables.get(varName);
		});
		...
		Lua_helper.add_callback(lua, "loadSong", function(song:String, difficulty:Int, ?isStory:Bool = false)
		{
			try
			{
				...
				WeekData.reloadWeekFiles(isStory);
