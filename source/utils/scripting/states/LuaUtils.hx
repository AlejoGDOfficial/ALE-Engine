...
class LuaUtils
{
	...
	public static function getModSetting(saveTag:String, ?modName:String = null)
	{
		...
		else
		{
			...
			ScriptState.instance.addTextToDebug('getModSetting: $path could not be found!', FlxColor.RED);
			#else
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		ScriptState.instance.addTextToDebug('getModSetting: "$saveTag" could not be found inside $modName\'s settings!', FlxColor.RED);
		#else
	...
	{
		return #if LUA_ALLOWED ScriptState.instance.modchartTexts.exists(name) ? ScriptState.instance.modchartTexts.get(name) : #end Reflect.getProperty(ScriptState.instance, name);
	}
	...
	public static function resetTextTag(tag:String) {
		...
		var target:FlxText = ScriptState.instance.modchartTexts.get(tag);
		target.kill();
	...
	public static function cameraFromString(cam:String):FlxCamera {
		switch (cam.toLowerCase().trim())
		{
			...
			case 'camhud', 'hud':
				return ScriptState.instance.camHUD;
			case 'camother', 'other':
				return ScriptState.instance.camOther;
		}
