...
import utils.scripting.substates.LuaUtils;
import utils.scripting.substates.LuaUtils.LuaTweenOptions;
#if SScript
import utils.scripting.substates.HScript;
#end
import utils.scripting.substates.DebugLuaText;
import utils.scripting.substates.ModchartSprite;
...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		Lua_helper.add_callback(lua, "setVar", function(varName:String, value:Dynamic) {
			ScriptSubstate.instance.variables.set(varName, value);
			return value;
		});
		Lua_helper.add_callback(lua, "getVar", function(varName:String) {
			return ScriptSubstate.instance.variables.get(varName);
		});
	...
	function oldTweenFunction(tag:String, vars:String, tweenValue:Any, duration:Float, ease:String, funcName:String)
	{
		...
		if(target != null) {
			ScriptSubstate.instance.modchartTweens.set(tag, FlxTween.tween(target, tweenValue, duration, {ease: LuaUtils.getTweenEaseByString(ease),
				onComplete: function(twn:FlxTween) {
					ScriptSubstate.instance.modchartTweens.remove(tag);
					ScriptSubstate.instance.callOnLuas('onTweenCompleted', [tag, vars]);
				}
			}));
		} else {
