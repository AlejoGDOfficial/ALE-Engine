...
class ReflectionFunctions
{
	...
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "createInstance", function(variableToSave:String, className:String, ?args:Array<Dynamic> = null) {
			...
			if(!ScriptSubstate.instance.variables.exists(variableToSave))
			{
				if(args == null) args = [];
				...
				if(obj != null)
					ScriptSubstate.instance.variables.set(variableToSave, obj);
				else
		...
		Lua_helper.add_callback(lua, "addInstance", function(objectName:String, ?inFront:Bool = false) {
			if(ScriptSubstate.instance.variables.exists(objectName))
			{
				var obj:Dynamic = ScriptSubstate.instance.variables.get(objectName);
				
				ScriptSubstate.instance.add(obj);
			}
