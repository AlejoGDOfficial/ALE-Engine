...
class ExtraFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "initSaveData", function(name:String, ?folder:String = 'psychenginemods') {
			if(!ScriptState.instance.modchartSaves.exists(name))
			{
				var save:FlxSave = new FlxSave();
				// folder goes unused for flixel 5 users. @BeastlyGhost
				save.bind(name, CoolUtil.getSavePath() + '/' + folder);
				ScriptState.instance.modchartSaves.set(name, save);
				return;
			}
		...
		Lua_helper.add_callback(lua, "flushSaveData", function(name:String) {
			if(ScriptState.instance.modchartSaves.exists(name))
			{
				ScriptState.instance.modchartSaves.get(name).flush();
				return;
			}
		...
		Lua_helper.add_callback(lua, "getDataFromSave", function(name:String, field:String, ?defaultValue:Dynamic = null) {
			if(ScriptState.instance.modchartSaves.exists(name))
			{
				var saveData = ScriptState.instance.modchartSaves.get(name).data;
		...
		Lua_helper.add_callback(lua, "setDataFromSave", function(name:String, field:String, value:Dynamic) {
			if(ScriptState.instance.modchartSaves.exists(name))
			{
				Reflect.setField(ScriptState.instance.modchartSaves.get(name).data, field, value);
				return;
			}
		...
		Lua_helper.add_callback(lua, "eraseSaveData", function(name:String)
		{
			if (ScriptState.instance.modchartSaves.exists(name))
			{
				ScriptState.instance.modchartSaves.get(name).erase();
				return;
			}
